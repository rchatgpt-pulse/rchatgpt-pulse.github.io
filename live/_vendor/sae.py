...
class SparseAutoencoder(nn.Module):
    ...
    def _normalized_mse(
        ...
        weights = sample_weights.view(-1, 1).expand_as(target)
        mse = F.mse_loss(pred, target, weight=weights)
        baseline_target = target.mean(dim=0, keepdim=True).expand_as(target)
        baseline_mse = F.mse_loss(baseline_target, target, weight=weights)
        if baseline_mse < 1e-8:
            print(f"Warning: near-zero baseline_mse={baseline_mse.item():.2e} (batch size={target.shape[0]}, "
                  f"target mean={target.mean().item():.4f}, target std={target.std().item():.2e}, "
                  f"target min={target.min().item():.4f}, target max={target.max().item():.4f})")
    ...
    def compute_loss(
        ...
        if self.prefix_lengths is None or len(self.prefix_lengths) == 1:
            main_l2 = self._normalized_mse(recon, x, sample_weights=sample_weights)
        else:
            l2_terms = []
            ...
            for end in self.prefix_lengths:
                ...
                l2_terms.append(
                    self._normalized_mse(prefix_recon, x, sample_weights=sample_weights)
                )
            main_l2 = torch.stack(l2_terms).mean()

        # multi-K term ------------------------------------------------------
        if multi_coef != 0 and info["multik_reconstruction"] is not None:
            multik_l2 = self._normalized_mse(
                info["multik_reconstruction"], x, sample_weights=sample_weights
            )
            main_l2 = main_l2 + multi_coef * multik_l2
        ...
        if self.aux_k is not None and info["aux_indices"] is not None:
            ...
            aux_loss = self._normalized_mse(err_recon, err, sample_weights=sample_weights)
            main_l2 = main_l2 + aux_coef * aux_loss
    ...
    def compute_reconstruction_error(
        ...
        sample_weights: Union[np.ndarray, torch.Tensor],
        feature_inds=None,
    ) -> float:
        """Compute normalized reconstruction error (MSE) on given embeddings.

        Args:
            embeddings: Input embeddings to reconstruct (numpy array or torch tensor)
            sample_weights: Per-sample weights for reconstruction loss
            feature_inds: Optional list of feature indices to use for reconstruction.
                          If provided, only these features will be used (others zeroed out).

        Returns:
            Normalized MSE (ratio of reconstruction MSE to baseline MSE)
        """
        self.eval()
        with torch.no_grad():
            ...
            sample_weights = torch.as_tensor(sample_weights, dtype=X.dtype, device=X.device)

            if feature_inds is None:
                # Use existing functionality via compute_loss
                recon, info = self(X)
                ...
                # Custom reconstruction with only selected features
                recon, info = self(X)
                activ = info["activations"]

                # Zero out activations for non-selected features
                mask = torch.zeros(self.m_total_neurons, device=self.device)
                mask[feature_inds] = 1.0
                activ_masked = activ * mask.unsqueeze(0)

                # Reconstruct using masked activations
                recon_masked = self.decoder(activ_masked) + self.input_bias
                
                return self._normalized_mse(recon_masked, X, sample_weights=sample_weights).item()
    ...
    def fit(
        ...
        X_train : torch.Tensor
            Training data.
        sample_weights : torch.Tensor
            Per-sample weights for training loss.
        X_val : torch.Tensor | None, optional
            Validation data for early stopping.
        save_dir : str | None, optional
            Directory to save the model checkpoint after training.
        batch_size : int, optional
            Batch size for training (default: 512).
        learning_rate : float, optional
            Learning rate (default: 5e-4). Use a lower value like 1e-4 when
            continuing training with skip_init=True.
        n_epochs : int, optional
            Number of training epochs (default: 200).
        aux_coef : float, optional
            Coefficient for auxiliary loss (default: 1/32).
        multi_coef : float, optional
            Coefficient for multi-K loss (default: 0.0).
        ...
        skip_init : bool, optional
            If True, skip weight initialization (default: False).
            Use this to continue training on new data while preserving
            learned features. Also resets dead neuron tracking.

        Returns
        -------
        dict
            Training history with 'train_loss', 'val_loss', 'dead_neuron_ratio'.
        """
        train_loader = DataLoader(
            TensorDataset(X_train, sample_weights), batch_size=batch_size, shuffle=True
        )
        ...
        if not skip_init:
            ...
            # Reset dead neuron tracking for new data
        ...
        for epoch in iterator:
            ...
            for batch_x, batch_w in train_loader:
                ...
                loss = self.compute_loss(
                    batch_x, recon, info, aux_coef, multi_coef, sample_weights=batch_w
                )
            ...
            # Validation (without temporal loss to get pure reconstruction metric)
            if val_loader is not None:
                self.eval()
                val_losses = []
                with torch.no_grad():
                    for batch_x, batch_w in val_loader:
                        batch_w = batch_w.to(self.device)
                        batch_x = batch_x.to(self.device)
                        recon, info = self(batch_x)
    ...
    def reorder_features(self, feature_order):
        """Reorder features (neurons) according to the given order.

        :param feature_order: list/array of indices specifying the new order of features
        """
        feature_order = torch.tensor(feature_order, dtype=torch.long, device=self.device)

        # Reorder encoder weights (input_dim x m_total_neurons)
        self.encoder.weight.data = self.encoder.weight.data[feature_order, :]

        # Reorder decoder weights (m_total_neurons x input_dim)
        self.decoder.weight.data = self.decoder.weight.data[:, feature_order]

        # Reorder neuron bias
        self.neuron_bias.data = self.neuron_bias.data[feature_order]

        # Reorder dead neuron tracking
        self.steps_since_activation = self.steps_since_activation[feature_order]
...
def load_model(path: str, device: str = "cuda" if torch.cuda.is_available() else "cpu") -> SparseAutoencoder:
    ckpt = torch.load(path, pickle_module=pickle, map_location = torch.device(device))
...
def train_sae(
    ...
    *,
    ...
    sample_weights: Union[List, np.ndarray],
    ...
    clip_grad: float = 1.0,
    show_progress: bool = True,
) -> Tuple[SparseAutoencoder, Optional[Dict]]:
    ...
    Args:
        ...
        continue_from_existing: If True and checkpoint exists, load it and continue training
                                on the new embeddings (skip weight initialization)
        val_embeddings: Optional validation embeddings for early stopping during SAE training
        sample_weights: Per-sample weights for training loss
    ...
    if checkpoint_dir is not None:
        ...
        print("looking for cached model at", checkpoint_path)
        if os.path.exists(checkpoint_path): 
            loaded_model = load_model(checkpoint_path)
            # If we're not overwriting and not explicitly continuing training,
            # just return the cached checkpoint.
            if (not overwrite_checkpoint) and (not continue_from_existing):
                print("Returning cached model without further training.")
    ...
    if loaded_model is not None:
        ...
        print(
            'Using loaded weights on current batch, error = ',
            sae.compute_reconstruction_error(embeddings, sample_weights),
        )
    ...
    hist = sae.fit(
        ...
        # If a checkpoint existed and we're continuing, preserve weights.
        skip_init=continue_from_existing and (loaded_model is not None),
    )
